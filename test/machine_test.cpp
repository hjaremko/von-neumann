...
TEST_CASE( "example codes executed successfully", "[example]" )
{
    ...
    SECTION( "array sum example" )
    {
        const std::vector<vnm::word> code = {
            vnm::word { "LOAD", "$", 0 },   vnm::word { "STORE", "$", 19 },
            vnm::word { "LOAD", "$", 21 },  vnm::word { "STORE", "$", 18 },
            vnm::word { "LOAD", "$", 21 },  vnm::word { "ADD", "@", 20 },
            vnm::word { "SUB", "@", 18 },   vnm::word { "JZERO", "$", 15 },
            vnm::word { "LOAD", "@", 19 },  vnm::word { "ADD", "&", 18 },
            vnm::word { "STORE", "$", 19 }, vnm::word { "LOAD", "@", 18 },
            vnm::word { "ADD", "$", 1 },    vnm::word { "STORE", "$", 18 },
            vnm::word { "JUMP", "$", 4 },
        };
        ...
        REQUIRE( *memory_state.at( vnm::word { 18 } ) == 26 );
        REQUIRE( *memory_state.at( vnm::word { 19 } ) == 150 );
    }
    ...
    SECTION( "double sum example" )
    {
        ...
        const auto memory_state = machine.get_memory();
        REQUIRE( *memory_state.at( vnm::word { 7 } ) == 110 );
    }
}
